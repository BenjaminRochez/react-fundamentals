...
We learned how to use interpolation to render JSX elements. 
Then we used React.createElement and passed the function we created earlier as the first parameter to create our first "component" that can be visible through the react devtool.
Finally, we combined the idea of JSX templates with react.createElement to produce a component <Message />
Be aware, this works because JSX specs says that if you start with an < and then a react element with a capital letter, it searches for a reference insthat is in scope. 
We validated the prop types of the component using propTypes. This propTypes check won't be included inside the react production mode as it impacts performances
This propTypes check has been replaced with a package to simplify our life (prop-types).
We also learned how to fix the fact that JSX needs one parent element only with React.Fragment
## Background
